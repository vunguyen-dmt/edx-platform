...
def settings_handler(request, course_key_string):  # lint-amnesty, pylint: disable=too-many-statements
    ...
    with modulestore().bulk_operations(course_key):
        ...
        if 'text/html' in request.META.get('HTTP_ACCEPT', '') and request.method == 'GET':
            ...
            log.info('isInstructor : ' + str(CourseInstructorRole(course_key).has_user(request.user)))
            log.info('isCourseStaff : ' + str(CourseStaffRole(course_key).has_user(request.user)))
            log.info('isGlobalStaff : ' + str(GlobalStaff.has_user(request.user)))
